...
class new(tkinter.Toplevel):
    def __init__(self,client ,data):
        ...
        tkinter.Label(f1, text='指令名字').pack(side=tkinter.LEFT);
        tkinter.Entry(f1,textvariable=self.post).pack()
        tkinter.Label(f2, text='指令路径').pack(side=tkinter.LEFT);
        tkinter.Entry(f2,textvariable=self.post1).pack()
        tkinter.Button(f3,text='确定',command=self.commit).pack();
        return
    def commit(self):
        data={"title":self.data['data']['title'],"obj":"Order" , "action":"do" ,"user":"root" ,"pid":self.data['pid'] ,"oid":self.data['oid'],"arg":[self.post.get(),self.post1.get()]}
